  ...
  if (loading && !email) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-3 text-slate-500">
        ...
        <span className="text-xs font-semibold font-mono">Mengotentikasi Dasbor Mahasiswa...</span>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col selection:bg-teal-500/20 selection:text-[#003057]">
      <StudentDashboard
        email={email}
      ...
      <ComparisonModal
        ...
        selectedComparisonGroup={selectedComparisonGroup}
      />

      <RequestModal
        isOpen={showRequestModal}
        onClose={() => setShowRequestModal(false)}
        onSuccess={() => fetchInspections(true)}
        mapReady={mapReady}
